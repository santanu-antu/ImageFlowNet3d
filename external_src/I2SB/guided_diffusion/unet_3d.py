...
# Adapted from the 2D UNet in unet.py for true isotropic 3D processing.
...
class Upsample3D(nn.Module):
    """
    A true 3D upsampling layer with an optional convolution.
    Upsamples all three spatial dimensions by a factor of 2.
    ...
    def forward(self, x):
        ...
        # True 3D upsampling: upsample all three spatial dimensions
...
class Downsample3D(nn.Module):
    """
    A true 3D downsampling layer with an optional convolution.
    Downsamples all three spatial dimensions by a factor of 2.
    ...
    def __init__(self, channels, use_conv, out_channels=None):
        ...
        self.out_channels = out_channels or channels
        self.use_conv = use_conv
        # True 3D stride: downsample all three dimensions
        stride = 2
        if use_conv:
            self.op = conv_nd(
                3, self.channels, self.out_channels, 3, stride=stride, padding=1
            )
        else:
...
class AttentionBlock3D(nn.Module):
    ...
    Note: For large volumes, attention can be memory-intensive as it scales
    with O((D*H*W)^2). Consider using smaller volumes or disabling attention.
    """
    ...
    def __init__(
        ...
        if use_new_attention_order:
            self.attention = QKVAttention3D(self.num_heads)
        else:
            self.attention = QKVAttentionLegacy3D(self.num_heads)
...
class QKVAttentionLegacy3D(nn.Module):
    """
...
class UNetModel3D(nn.Module):
    """
    The full 3D UNet model with attention and timestep embedding.
    Designed for true volumetric 3D data processing.
    ...
    def __init__(
            ...
            for i in range(num_res_blocks + 1):
                ...
                if level and i == num_res_blocks:
                    ...
                    layers.append(
                        ...
                        if resblock_updown
                        else Upsample3D(ch, conv_resample, out_channels=out_ch)
                    )
                    ds //= 2
        ...
        self.out = nn.Sequential(
            normalization(ch),
            nn.SiLU(),
            zero_module(conv_nd(3, input_ch, out_channels, 3, padding=1)),
        )
