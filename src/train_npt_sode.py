...
from nn.unet_sode import SODEUNet
...
def train_epoch(config: AttributeHashmap,
    ...
    for iter_idx, (images, timestamps) in enumerate(tqdm(train_set)):
        ...
        x_start_noisy, x_end_noisy = x_noisy_list[0], x_noisy_list[1]
...
def test(config: AttributeHashmap):
    ...
    for best_type in ['pred_psnr', 'seg_dice']:
        ...
        test_input_time, test_pred_time = [], []
        ...
        for iter_idx, (images, timestamps) in enumerate(tqdm(test_set)):
            ...
            test_input_time.append(','.join([t_arr[i].item() for i in range(len(t_arr)-1)]))
            test_pred_time.append(t_arr[-1].item())
            ...
            deltaT_list.append((t_arr[-1] - t_arr[0]).item())
        ...
        results_df = pd.DataFrame({
            'input time': test_input_time,
            'pred time': test_pred_time,
...
def plot_side_by_side(t_arr, x0_true, xT_true, x0_recon, xT_recon, xT_pred, save_path: str,
    ...
    ax.set_title('GT(t=0), time: %s\n[vs GT(t=T)]: PSNR=%.2f, SSIM=%.3f' % (
        t_arr[0].item(), psnr(x0_true, xT_true), ssim(x0_true, xT_true)))
    ax.set_axis_off()
    ax.set_aspect(aspect_ratio)
    ax = fig_sbs.add_subplot(2, 6, 7)
    ax.imshow(xT_true)
    ax.set_title('GT(t=T), time: %s' % t_arr[-1].item())
    ax.set_axis_off()
    ax.set_aspect(aspect_ratio)
    ...
    ax.imshow(x0_recon)
    ax.set_title('Recon(t=0), time: %s' % t_arr[0].item())
    ax.set_axis_off()
    ax.set_aspect(aspect_ratio)
    ax = fig_sbs.add_subplot(2, 6, 8)
    ax.imshow(xT_recon)
    ax.set_title('Recon(t=T), time: %s' % t_arr[-1].item())
    ax.set_axis_off()
    ax.set_aspect(aspect_ratio)
    ...
    ax.set_title('Pred(t=T), time: %s -> time: %s\n[vs GT(t=T)]: PSNR=%.2f, SSIM=%.3f' % (
        [t_arr[i].item() for i in range(len(t_arr)-1)], t_arr[-1].item(), psnr(xT_pred, xT_true), ssim(xT_pred, xT_true)))
    ax.set_axis_off()
    ax.set_aspect(aspect_ratio)
    ...
    ax.set_title('|GT(t=0) - GT(t=T)|, time: %s and %s\n[MAE=%.4f, MSE=%.4f]' % (
        t_arr[0].item(), t_arr[-1].item(), np.mean(np.abs(x0_true - xT_true)), np.mean((x0_true - xT_true)**2)))
    ax.set_axis_off()
    ax.set_aspect(aspect_ratio)
    ax = fig_sbs.add_subplot(2, 6, 10)
    ax.imshow(np.abs(xT_pred - xT_true))
    ax.set_title('|Pred(t=T) - GT(t=T)|, time: %s\n[MAE=%.4f, MSE=%.4f]' % (
        t_arr[-1].item(), np.mean(np.abs(xT_pred - xT_true)), np.mean((xT_pred - xT_true)**2)))
    ax.set_axis_off()
    ax.set_aspect(aspect_ratio)
    ...
    if x0_true_seg is not None:
        ...
        ax.set_title('GT(t=0), time: %s\n[vs GT(t=T)]: DSC=%.3f, HD=%.2f' % (
            t_arr[0].item(), dice_coeff(x0_true_seg, xT_true_seg), hausdorff(x0_true_seg, xT_true_seg)))
    ax.set_axis_off()
    ax.set_aspect(aspect_ratio)
    ...
    if xT_pred_seg is not None:
        ...
        ax.imshow(xT_pred)
        ax.set_title('Pred(t=T), time: %s -> time: %s\n[vs GT(t=T)]: DSC=%.3f, HD=%.2f' % (
            [t_arr[i].item() for i in range(len(t_arr)-1)], t_arr[-1].item(), dice_coeff(xT_pred_seg, xT_true_seg), hausdorff(xT_pred_seg, xT_true_seg)))
    ax.set_axis_off()
    ax.set_aspect(aspect_ratio)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--model', default='SODEUNet', type=str)
