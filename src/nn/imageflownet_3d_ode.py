...
from .nn_utils_3d import PPODEfunc3D, ODEBlock3D
...
class ImageFlowNet3DODE(BaseNetwork):
    ...
    def __init__(self,
                 ...
                 **kwargs):
        '''
        3D ImageFlowNet model with ODE for volumetric data.
        NOTE: This is a 3D UNet with a position-parameterized ODE vector field.
        ...
        volume_size: int
            Size of the input volume (assumes cubic: D=H=W).
        ode_location: str
        ...
        # because running NeuralODE requires significant GPU space.
        # For 3D, we use even smaller settings.
        self.unet = create_model_3d(
        ...
        # avoid channel mismatch (e.g., synthetic dataset uses 1 channel for 3D).
        h_dummy = torch.zeros((1, in_channels, volume_size, volume_size, volume_size)).type(self.unet.dtype)
        ...
        # Construct the ODE modules using 3D versions.
        self.ode_list = torch.nn.ModuleList([])
        if self.ode_location == 'bottleneck':
            self.ode_list.append(ODEBlock3D(PPODEfunc3D(dim=h_dummy_bottleneck.shape[1])))
        ...
        if self.contrastive:
            pred_dim = 256
            # For 3D, flatten all spatial dimensions: C * D * H * W
            self.projector = torch.nn.Sequential(
                torch.nn.Linear(h_dummy_bottleneck.shape[1] *
                                h_dummy_bottleneck.shape[2] *
                                h_dummy_bottleneck.shape[3] *
                                h_dummy_bottleneck.shape[4], pred_dim)
            )
    ...
    def forward(self, x: torch.Tensor, t: torch.Tensor, return_grad: bool = False):
        ...
        # Provide a dummy time embedding, since we are learning a position-parameterized ODE vector field.
